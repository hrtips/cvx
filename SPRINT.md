...
### Custom GPT integration (landed, unreleased)

CVX is reachable from a ChatGPT Custom GPT: an Action returns the zipped bundle
as an `openaiFileResponse`, ChatGPT materialises it for Code Interpreter, and it
runs. Endpoints are static files generated by `scripts/gpt-endpoints.js` and
served from Pages, so there is no service to operate, and neither the Action
schema nor the GPT instructions name a version — a release needs no change to
the GPT. Setup and the paste-ready instructions are in
[docs/custom-gpt.md](docs/custom-gpt.md); `site/privacy.html` is the project
privacy statement a shareable Action requires.

**GPT Actions were on ARCHITECTURE.md §7.3's cut list. The maintainer overrode
that on 2026-08-18** ("disregard the architecture cut list, we need the UX
sorted"). Container images and standalone executables stay cut — see the last
