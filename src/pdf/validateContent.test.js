...
function makeDir(files = {}, { layouts, images } = {}) {
  const dir = mkdtempSync(join(tmpdir(), 'cvx-validate-'))
  dirsToClean.push(dir)
  for (const [name, content] of Object.entries(files)) writeFileSync(join(dir, name), content)
  if (layouts) {
    mkdirSync(join(dir, 'layouts'), { recursive: true })
    for (const [name, content] of Object.entries(layouts)) writeFileSync(join(dir, 'layouts', name), content)
  }
  if (images) {
    mkdirSync(join(dir, 'images'), { recursive: true })
    for (const [name, content] of Object.entries(images)) writeFileSync(join(dir, 'images', name), content ?? '')
  }
  return dir
}
...
describe('validateContent — happy path', () => {
  it('accepts a minimal valid content dir under --strict', () => {
    ...
    expect(res.checked).toEqual(expect.arrayContaining(['personal.yaml', 'summary.yaml', 'experience.yaml']))
  })
})
...
describe('validateContent — structural problems', () => {
  ...
  it('reports a YAML parse error', () => {
    const res = validateContent({ contentDir: makeDir({ ...BASE, 'education.yaml': '"unterminated string\n' }), fontsDir: FONTS })
...
describe('validateContent — schema findings', () => {
  it('reports a missing required field as a schema error', () => {
    const res = validateContent({ contentDir: makeDir({ ...BASE, 'personal.yaml': 'title: No Name\n' }), fontsDir: FONTS })
    expect(res.ok).toBe(false)
  ...
  it('picks the matching oneOf branch for an invalid bullet object', () => {
    const experience = dump([{ role: 'R', company: 'C', period: 'p', bullets: [{ link: { href: 'https://x', label: 'y' } }] }])
    const res = validateContent({ contentDir: makeDir({ ...BASE, 'experience.yaml': experience }), fontsDir: FONTS })
    expect(res.ok).toBe(false)
...
describe('validateContent — real-metric checks', () => {
  it('warns when a forced page1ExperienceCount cannot fit', () => {
    const experience = dump([
      { role: 'R', company: 'C', period: 'p', bullets: Array.from({ length: 20 }, () => 'A long bullet line that spans a good fraction of the available column width for measurement purposes.') },
    ])
    const summary = dump(Array.from({ length: 5 }, () => 'A long summary sentence used to consume vertical space on page one so the overflow estimate triggers.'))
    const res = validateContent({
      contentDir: makeDir({ 'personal.yaml': 'name: X\n', 'summary.yaml': summary, 'experience.yaml': experience, 'config.yaml': dump({ page1ExperienceCount: 1 }) }),
      fontsDir: FONTS,
    })
...
describe('validateContent — photo probe', () => {
  it('warns when the only image has an unsupported extension', () => {
    const res = validateContent({ contentDir: makeDir(BASE, { images: { 'profile.txt': '' } }), fontsDir: FONTS })
  ...
  it('warns when an image is present but none is named profile', () => {
    const res = validateContent({ contentDir: makeDir(BASE, { images: { 'random.png': '' } }), fontsDir: FONTS })
    const finding = res.warnings.find((f) => f.code === 'no-photo')
    expect(finding).toBeTruthy()
  ...
  it('does not warn when a valid profile photo exists', () => {
    const res = validateContent({ contentDir: makeDir(BASE, { images: { 'profile.jpg': '' } }), fontsDir: FONTS })
