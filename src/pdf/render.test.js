...
import { renderCV, discoverLayouts } from './render.js'
...
function writeContent(bag, { config, layouts } = {}) {
  ...
  if (layouts) {
    ...
    for (const [name, doc] of Object.entries(layouts)) writeFileSync(join(ld, `${name}.yaml`), dump(doc))
  }
  return dir
}
...
const RICH = {
  ...
  summary: [
    ...
    { text: 'Publishes regularly, e.g. ', link: { href: 'https://ada.example.com/blog', label: 'the engineering blog' }, suffix: '.' },
  ],
  experience: [
    {
      ...
      bullets: [
        ...
        { text: 'Shipped the ', link: { href: 'https://example.com/x', label: 'Difference Engine v2' }, suffix: ' platform to general availability.' },
  ...
  referees: [
    { name: 'Charles Babbage', title: 'Founder', company: 'Babbage Systems', email: 'charles@example.com', phone: '+44 20 7946 0001' },
...
describe('renderCV — designed (two-column)', () => {
  it('renders the real Bruce Wayne template with a profile photo', async () => {
  ...
  it('renders rich content that forces a single continuation page (coral, split bullets)', async () => {
    const dir = writeContent(RICH, { config: RICH_CONFIG })
    const { buffer, themeName } = await renderCV({ contentDir: dir, fontsDir: FONTS, env: {}, warn: () => {} })
    expect(isPdf(buffer)).toBe(true)
    expect(themeName).toBe('coral')
  }, RENDER_TIMEOUT)

  it('renders enough content to span several continuation pages (cont + last page kinds)', async () => {
    const experience = Array.from({ length: 20 }, (_, i) => ({
      role: `Role Number ${i}`,
      company: `Company ${i}`,
      period: '2005 – Present',
      description: 'A description line for this role that adds a little vertical height to each packed entry.',
      bullets: Array.from({ length: 5 }, (_, j) => `Entry ${i} bullet ${j}: a reasonably long achievement sentence that wraps across the column and consumes vertical space.`),
    }))
    const dir = writeContent(
      { personal: { name: 'Many Pages' }, summary: ['A short summary.'], experience, education: [{ degree: 'BSc', institution: 'Uni' }], referees: [{ name: 'Ref' }] },
      { config: { schemaVersion: 1, theme: 'teal', layout: 'two-column' } }
    )
    const { buffer } = await renderCV({ contentDir: dir, fontsDir: FONTS, env: {}, warn: () => {} })
    expect(isPdf(buffer)).toBe(true)
  }, RENDER_TIMEOUT)

  it('renders a two-column CV with no summary (summary section renders nothing)', async () => {
    const dir = writeContent(
      {
  ...
  it('folds sidebar sections onto page 1 for a single-page CV and honours a user layout file', async () => {
    vi.spyOn(console, 'warn').mockImplementation(() => {})
    const dir = writeContent(
      {
        personal: { name: 'Grace Hopper', title: 'Rear Admiral' },
        summary: ['Short summary line.'],
        experience: [{ role: 'Engineer', company: 'Navy', period: '1944', bullets: ['One concise bullet.'] }],
      ...
      },
      {
        ...
        layouts: {
          mylayout: {
            ...
            pages: {
              first: {
                sidebar: ['identity-photo', 'contact', { education: {} }, { experience: { continued: true } }],
...
describe('renderCV — single-column (header-ats section + template)', () => {
  it('renders the real template in a single column (photo + company header)', async () => {
    cpSync(TEMPLATE, join(tmp, 'cv-content'), { recursive: true })
    writeFileSync(join(tmp, 'cv-content', 'config.yaml'), dump({ schemaVersion: 1, theme: 'teal', layout: 'single-column' }))
    const { buffer, filename, layoutName } = await renderCV({ contentDir: join(tmp, 'cv-content'), fontsDir: FONTS, env: {}, warn: () => {} })
    expect(isPdf(buffer)).toBe(true)
    expect(layoutName).toBe('single-column')
    expect(filename).toBe('bruce-wayne-ats.pdf')
  }, RENDER_TIMEOUT)

  it('renders a single-column CV with no company and a label-less link', async () => {
    const dir = writeContent(
      {
        personal: { name: 'Column Person', title: 'Analyst', email: 'c@x.com', location: 'Remote', links: [{ label: 'Site', href: 'https://s' }, { href: 'https://nolabel' }] },
        summary: ['s'],
        experience: [{ role: 'R', company: 'C', period: 'p', bullets: ['b'] }],
      },
      { config: { schemaVersion: 1, layout: 'single-column' } }
    )
    const { buffer, filename } = await renderCV({ contentDir: dir, fontsDir: FONTS, env: {}, warn: () => {} })
    expect(isPdf(buffer)).toBe(true)
...
describe('renderCV — ATS document (ats: true)', () => {
  it('renders the real template as an ATS document', async () => {
    cpSync(TEMPLATE, join(tmp, 'cv-content'), { recursive: true })
    const { buffer, filename, themeName, layoutName } = await renderCV({
      contentDir: join(tmp, 'cv-content'),
      fontsDir: FONTS,
  ...
  it('renders rich content (all ATS sections, object bullets, referee variants)', async () => {
    const dir = writeContent(RICH, { config: RICH_CONFIG })
    const { buffer } = await renderCV({ contentDir: dir, fontsDir: FONTS, ats: true, env: {}, warn: () => {} })
    expect(isPdf(buffer)).toBe(true)
  }, RENDER_TIMEOUT)

  it('renders a minimal ATS document (no company, no photo, no optional sections)', async () => {
    const dir = writeContent({ personal: { name: 'Solo Name', title: 'Widget Maker' } })
    const { buffer, filename } = await renderCV({ contentDir: dir, fontsDir: FONTS, ats: true, env: {}, warn: () => {} })
    expect(isPdf(buffer)).toBe(true)
...
describe('renderCV — warnings and errors', () => {
  ...
  it('throws on an unknown theme', async () => {
    const dir = writeContent(
      { personal: { name: 'X' }, summary: ['s'], experience: [{ role: 'R', company: 'C', period: 'p', bullets: ['b'] }] },
      { config: { schemaVersion: 1, theme: 'ultraviolet' } }
    )
    await expect(renderCV({ contentDir: dir, fontsDir: FONTS, env: {}, warn: () => {} })).rejects.toThrow(/Unknown theme/)
  })

  it('warns and falls back when the configured layout is not found', async () => {
    const dir = writeContent(
      { personal: { name: 'X' }, summary: ['s'], experience: [{ role: 'R', company: 'C', period: 'p', bullets: ['b'] }] },
      { config: { schemaVersion: 1, layout: 'ghost' } }
    )
    const warnings = []
    const { buffer } = await renderCV({ contentDir: dir, fontsDir: FONTS, env: {}, warn: (m) => warnings.push(m) })
    expect(isPdf(buffer)).toBe(true)
    expect(warnings.some((m) => m.includes('ghost') && /not found/i.test(m))).toBe(true)
  }, RENDER_TIMEOUT)

  it('warns when a forced page1ExperienceCount cannot fit', async () => {
    const bigBullets = Array.from({ length: 6 }, (_, i) => `Delivered a substantial and detailed accomplishment number ${i} spanning many teams, quarters and stakeholders across the organisation.`)
    const experience = Array.from({ length: 3 }, (_, i) => ({
      role: `Very Senior Role Number ${i}`,
      company: `Company ${i}`,
      period: '2020 – Present',
      description: 'A long paragraph describing the scope and responsibilities of this role in considerable detail for measurement.',
      bullets: bigBullets,
    }))
    const dir = writeContent(
      { personal: { name: 'Over Flow' }, summary: ['One reasonably long summary sentence used to eat into the page-1 budget for the overflow estimate.'], experience },
    ...
    expect(warnings.some((m) => m.includes('page1ExperienceCount') && m.includes('safety margin'))).toBe(true)
  }, RENDER_TIMEOUT)

  it('warns about characters the bundled font cannot render', async () => {
    const dir = writeContent({
...
describe('discoverLayouts', () => {
  ...
  it('loads and normalizes user layout files', () => {
    ...
    mkdirSync(ld, { recursive: true })
    writeFileSync(join(ld, 'custom.yaml'), dump({ template: 'two-column', pages: { first: { main: ['summary'] } } }))
