...
for (const [srcDir, libDir] of TREES) {
  for (const file of walk(srcDir)) {
    ...
    const retargeted = code.replace(/(from\s*["'][^"']+)\.jsx(["'])/g, '$1.js$2')
    // lib/ is a generated transform of src/ — src is the type-checked source of
    // truth, so mark the artifact @ts-nocheck to keep it out of `tsc` (bin/cvx.js
    // imports lib/ at runtime, which would otherwise drag the whole tree in).
    const rewritten = `// @ts-nocheck — generated by scripts/build-lib.js from src/; type-check src/, not this file\n${retargeted}`
