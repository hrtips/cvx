...
async function validate({ strict, json }) {
  const { validateContent } = /** @type {typeof import('../src/pdf/validateContent.js')} */ (
    await import('../lib/pdf/validateContent.js')
  )
...
async function list({ kind, json }) {
  const { discoverThemes } = /** @type {typeof import('../src/pdf/themes/index.js')} */ (
    await import('../lib/pdf/themes/index.js')
  )
...
async function build({ ats, json }) {
  const { renderCV } = /** @type {typeof import('../src/pdf/render.js')} */ (
    await import('../lib/pdf/render.js')
  )
  const warnings = []
    ...
    try {
      ...
      if (json)
        emit({
          ...
          all: true,
          ok: false,
          error: { code: 'render-failed', message: `${label} variant failed: ${err.message}` }
        })
...
try {
  ...
  if (values.version) {
    ...
    if (positionals[1] === 'init') {
      ...
      const { runMcpServer } = /** @type {typeof import('../src/mcp/server.js')} */ (
  ...
  if (jsonMode)
    emit({
      command,
      ok: false,
      error: { code: command === 'build' ? 'render-failed' : 'usage', message: err.message }
    })
