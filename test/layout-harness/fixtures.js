...
import { greedyPairwiseCover, allPairs, cartesianSize } from './pairwise.js'
...
function pairwiseRowToSpec(row, i) {
  ...
  return {
    ...
    description: `pairwise: ${Object.entries(sections).map(([k, v]) => `${k}=${v}`).join(',')}, textLength=${textLength}, volume=${volume}`,
    sections,
    textLength,
    volume,
  }
  ...
  return [
    {
      ...
      description: 'tall sidebar (every optional section "many") + short main (short text, fits-1-page volume) — the exact shape that triggers bug (a): sidebar overflows a physical page while the main column has long since run out.',
      sections: { certifications: 'many', publications: 'many', languages: 'many', referees: 'many', achievements: 'many' },
      textLength: 'short',
      volume: 'fits-1-page',
    },
    {
      id: 'risk-maxed-out',
      description: 'maxed out: every optional section "many", overflowing text, multi-page volume — worst-case content load (akin to the Batman demo).',
      sections: { certifications: 'many', publications: 'many', languages: 'many', referees: 'many', achievements: 'many' },
    ...
    },
    {
      ...
      description: 'sparse: every optional section absent, short text, fits-1-page volume — the opposite extreme (near-empty CV; checks resolveFirstSidebar\'s single-page fold does not misbehave when there is almost nothing to fold).',
      sections: { certifications: 'absent', publications: 'absent', languages: 'absent', referees: 'absent', achievements: 'absent' },
      textLength: 'short',
      volume: 'fits-1-page',
    },
  ]
  ...
  return [
    {
      ...
      description: 'single oversized section: certifications has ~60 items, everything else at "one" — does an individual sidebar section this large ever clip, or only waste pages? (untested territory per C0\'s own analysis — see research/c0-baseline.md)',
      sections: { certifications: 'one', publications: 'one', languages: 'one', referees: 'one', achievements: 'one' },
      textLength: 'typical',
      volume: 'fits-1-page',
    ...
    {
      id: 'edge-minimal',
      description: 'all-optional-absent minimal CV: only personal + summary + experience (no education, competencies, or any of the five optional sections).',
      minimal: true,
      textLength: 'short',
      volume: 'fits-1-page',
    },
    {
      ...
      description: 'one-entry sections: all five optional sections simultaneously at "one" — pairwise coverage guarantees every PAIR of sections is jointly "one", not all five at once, so this is added explicitly.',
      sections: { certifications: 'one', publications: 'one', languages: 'one', referees: 'one', achievements: 'one' },
      textLength: 'typical',
      volume: 'fits-1-page',
    },
    {
      ...
      description: 'a label-less long-URL link in personal.links (renders as the raw href — ContactSection.jsx: `l.label || l.href`) — stresses long-token wrap in a real render path, not just the isolated measure-diff corpus.',
      sections: { certifications: 'one', publications: 'one', languages: 'one', referees: 'one', achievements: 'one' },
      textLength: 'typical',
      volume: 'fits-1-page',
    ...
    },
    {
      ...
      description: 'non-Latin personal.name (Sinhala) — ties to the fallback-font measurement risk (design doc G-a): Lato has no Sinhala glyphs and no fallback (e.g. Noto) is registered today.',
    ...
    },
    {
      ...
      description: 'referees: [] (explicit empty list) rather than absent (no file) — schema-documented distinct shape (empty list prints the "available upon request" placeholder in the two-column layout).',
      sections: { certifications: 'one', publications: 'one', languages: 'one', achievements: 'one' },
    ...
    },
    {
      ...
      description: 'config-driven page1ExperienceCount + page1SplitBullets (mirrors the shipped scaffold\'s own config.yaml) — a distinct packExperiences() branch with no budget check of its own.',
      sections: { certifications: 'one', publications: 'one', languages: 'one', referees: 'one', achievements: 'one' },
      textLength: 'typical',
      volume: 'multi-page',
...
export function buildFixturePlan() {
  ...
  if (duplicateIds.length > 0) throw new Error(`buildFixturePlan: duplicate fixture ids: ${duplicateIds.join(', ')}`)
  ...
  return {
    ...
    meta: {
      pairwise: {
        ...
        droppedReason: 'Full cartesian coverage is 1,944 combinations at ~1s/fixture-variant to render+rasterize — not viable for a CI-gated suite. Reduced to a greedy pairwise cover (every (factor,level) pair touches every other pair at least once) — the standard combinatorial-testing argument that pairwise coverage catches the large majority of interaction defects at a small fraction of full-cartesian cost. Computed deterministically (no RNG — see pairwise.js), not hand-picked.',
      },
      riskScenarioCount: risks.length,
      namedEdgeCaseCount: edgeCases.length,
      totalFixtures: fixtures.length,
      variantAxisNote: '"variant: designed|ats" from the sprint\'s cartesian is not multiplied into the fixture count — every fixture is rendered through both variants via scaffold.js\'s buildAll() (two separate CLI processes — see that file\'s docblock for why not the batched `cvx build --all`), so variant coverage is complete without doubling the fixture list.',
    },
  }
}
...
export function describeFixturePlan(meta) {
  const lines = []
  lines.push(`Full cartesian: ${meta.pairwise.totalCartesianCombinations} combinations (${Object.entries(meta.pairwise.factors).map(([k, v]) => `${k}:${v.length}`).join(' x ')}).`)
  lines.push(`Pairwise cover: ${meta.pairwise.rowsGenerated} fixtures satisfy all ${meta.pairwise.totalPairsRequired} required pairs (dropped ${meta.pairwise.droppedCombinations} of ${meta.pairwise.totalCartesianCombinations} raw combinations — ${meta.pairwise.droppedReason}).`)
  lines.push(`Plus ${meta.riskScenarioCount} named risk scenarios (tall-sidebar+short-main, maxed-out, sparse-1-page) and ${meta.namedEdgeCaseCount} named edge cases.`)
