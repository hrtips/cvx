...
download.** No engine change; the artifact set grew.

`5.3 MB` is awkward to move into a sandbox, and the two ways to shrink it help
very differently — measured, not guessed:

| | raw | zipped |
|---|---|---|
| `cvx-1.9.2.bundle.js` | 5.33 MB | 1.09 MB |
| `cvx-1.9.2.bundle.min.js` | 2.62 MB | 0.92 MB |

**Zipping is the big lever (~5×); minifying only adds ~16% on top of it** —
because the fonts, schema and template already sit inside the file as gzipped
base64 and cannot compress twice. Minifying *does* halve the raw file, which is
what matters when something wants the `.js` itself rather than an archive, such
as a Custom GPT Knowledge entry. Take the plain build otherwise: it keeps
readable stack traces, which matter most in the offline sandboxes this bundle
exists for. Both render byte-identical PDFs — asserted by a test, not assumed.

Every variant is published under a versioned **and** an unversioned name, so
`releases/latest/download/cvx.bundle.min.js.zip` and its three siblings are all
stable URLs, and `SHA256SUMS.txt` now covers all eight files. It is generated by
the build rather than by shell in CI, so the same file appears locally and on the
release, and CI re-verifies it with coreutils — a different implementation than
the one that wrote it.

The zips are written by a small ZIP encoder in `scripts/build-standalone.js`
rather than by the `zip` binary, which does not exist on the Windows CI leg that
runs this script. Entry timestamps are pinned to the ZIP epoch so rebuilds are
byte-stable; each archive is verified to round-trip to its source, and the tests
re-read them through an independent header walk.
